...
//---------𝗥𝗘𝗗𝗘𝗦 𝗦𝗢𝗖𝗜𝗔𝗟𝗘𝗦---------
const soporteTurbo = 'https://youtube.com/c/turbontr1?sub_confirmatión=1' 
const instaTurbo = 'https://youtube.com/c/turbontr1?sub_confirmatión=1'
//--anti delete 
antidel = false
ngetik = false
vn = true
...
const linkwa = 'https://youtube.com'
const linkwa = 'https://chat.whatsapp.com'
    ...
    if (budy.includes(`https://chat.whatsapp.com/Ly4I2LObSvW8VgOnJjoTurboA`)) return reply('✳️ Menos mal que este enlace es del grupo *📲💻ANDROID WORLD🎬🎮* v:')
    if (budy.includes(`https://chat.whatsapp.com/G5sXrkhJ0pb0Tu8nhWLaFK`)) return
		...
		setTimeout( () => {
		reply('Eliminado por mandar enlaces 🐈💨') 
		}, 1100)
		setTimeout( () => {
...
//---colores 
 ...
 if (!isGroup && isCmd) console.log(color('[Turbo98]','aqua'), "Pv", color(command, "blue"), "de", (sender.split('@')[0]),  args.length)
if (isGroup && isCmd) console.log(color('[Turbo98]','aqua'), "Gp", color(command, "green"), "de", (sender.split('@')[0]), "en", (groupName), args.length)
...
if (!isVerify) return reply(userB(prefix))
  if (isBanned) return reply(banf())
const Turbonum = `${owner}@s.whatsapp.net`
...
infobot =`⊷ *TURBO⚡BOT*
▢ Nombre : *${Turbo.user.name}*
▢ Version Bot : *4.0.0*
▢ Prefijo : *${single ? `${prefa}` : multi ? 'Multiprefijo' : 'SinPrefijo'}*
▢ Total Hits : *${totalhit}*
▢ Usuarios registrados : ${_user.length}
▢ Navegador : *${Turbo.browserDescription[1]}*
▢ Versión Navegador: *${Turbo.browserDescription[2]}*
▢ Velocidad : *${latensi2.toFixed(4)}*
▢ Tiempo Ejecucion : *${kyun(runtime2)}*
▢ Version Wha : ${Turbo.user.phone.wa_version}
▢ Grupo del Bot : 
*${soporteTurbo}*
break
...
⚠️Consecuencias al romper las reglas⚠️
...
case 'register':
case 'daftar':
			if (isVerify) return replyTurbo('✳️ Tu cuenta ya esta registrada')
					_user.push(sender)
fs.writeFileSync('./database/verify.json', JSON.stringify(_user))
try {
...
Turbo.sendMessage(from, daftarimg, image, {quoted: mek, caption: capt})
break
...
if (!isVerify) return reply(userB(prefix))
  if (isBanned) return reply(banf())
				...
				buff = `https://api.zeks.me/api/magernulis?apikey=WRhywqyYC0doYNeqTZymcB2SjrT&nama=Turbo98&kelas=6-08&text=${q}&tinta=6`
...
var mantap2 = await convertSticker(bas642, `💎 @Turbo98._`, `➣᳆⃟✪𝐓𝐔𝐑𝐁𝐎 ɃØ₮✪᳆͜͡➣`)
...
var mantap2r = await convertSticker(bas642r, `💎 @Turbo98._`, `➣᳆⃟✪𝐓𝐔𝐑𝐁𝐎 ɃØ₮✪᳆͜͡➣`)
...
var mantap2c = await convertSticker(bas642c, `💎 @Turbo98._`, `➣᳆⃟✪𝐓𝐔𝐑𝐁𝐎 ɃØ₮✪᳆͜͡➣`)
...
if (args.length < 1) return reply(`✳️ Ingrese el link de un video`)
		...
		hx.ttdownloader(`${args[0]}`)
    		...
    		.then(async (a) => {
    		me = `✅ Aquí tienes, No olvides suscribirte a mi canal de YouTube Turbo ntr`
...
┃
├➽📤 La música se está
├➽ Enviando, si no llega
├➽ Descargue por el link
...
d = []
teks = '❰❰✪TOP 10 CULOS✪❱❱\n Está es una lista de los más culos del grupo\n╭─✦─•❮✦⋆☬⋆✦❯•─✦─╮\n' 
for(i = 0; i < 10; i++) {
...
//--- on/off antilink WhatsApp 
  ...
  if (isBanned) return reply(banf())
				if (!isGroup) return reply(group())
					if (!isGroupAdmins && !isOwner) return reply(admin())
					if (!isBotGroupAdmins) return reply(Badmin())
					if (args.length < 1) return reply(`☣️ ANTILINK WHATSAPP ☣️\n\n*${prefix + command} on* para activar\n*${prefix + command} off* para desactivar`)
					if ((args[0]) === 'on') {
...
case 'profile':
					...
					ppimg = 'https://i.ibb.co/PZNv21q/Profile-Turbo98.jpg'
					}
					 profile = `╭─✦─•❮✦⋆☬⋆✦❯•─✦─╮
╽          ❰❰✪PERFIL✪❱❱
├➽🔖 Nombre: ${pushname}
