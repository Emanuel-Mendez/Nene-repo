...
case 'bienvenida':
					  ...
					  if (Number(args[0]) === on) {
						  if (isWelcom) return reply('Ya esta activo')
						  welcom.push(from)
						  fs.writeFileSync('./database/welcom.json', JSON.stringify(welcom))
						  reply('Se activo con exito')
					  } else if (Number(args[0]) === off) {
						  welcom.splice(from, 1)
						  fs.writeFileSync('./database/welcom.json', JSON.stringify(welcom))
						  reply('Se desactivo con exito')
					  } else {
						  reply('on para activar, off desactivar')
					  }
                     break
